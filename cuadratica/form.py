...
#    Feb 14, 2019 11:58:45 AM EST  platform: Windows NT
...
class Ecuacion_cuadratica:
    def __init__(self, top=None):
        ...
        self.titulo.configure(text='''Ecuacion Cuadratica''')
        self.titulo.configure(width=247)

        #funciones para mathFunctions----------------------------------------------------------------
        def Calcular():

            def var_a():
                a = self.component_a.get()
                if(a.isdigit()):
                    return round(float(a),2)
                else:
                    print("A no es digito")
            def var_b():
                b = self.component_b.get()
                if(b.isdigit()):
                    return round(float(b),2)
                else:
                    print("B no es digito")
            def var_c():
                c = self.component_c.get()
                if(c.isdigit()):
                    return round(float(c),2)
                else:
                    print("C no es digito")
            
            disc = mf.opDisc(var_a(),var_b(),var_c())

            sol_1 = mf.sol_x1(var_b(),var_a(),disc)
            self.sol_x1.delete(0,len(self.sol_x1.get()))
            self.sol_x1.insert(0,sol_1)
            sol_2 = mf.sol_x2(var_b(),var_a(),disc)
            self.sol_x2.delete(0,len(self.sol_x1.get()))
            self.sol_x2.insert(0,sol_2)
        ...
        self.btn_Calcular.configure(command=lambda:Calcular())
        ...
        def hacerGrafico():
            ...
            def var_c():
                ...
                if(c.isdigit()):
                    ...
                    print("C no es digito")
            x = mf.xVertice(var_b(),var_a())
            y = mf.yVertice(var_b(),var_a(),var_c())
            gf.graficar(var_a(),var_b(),var_c())
        ...
        self.btnGraficar.configure(command = lambda: hacerGrafico())
